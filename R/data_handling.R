...
#' Collate one local Vortex genetics output file into a data.frame
#'
#' \code{collate_one_gen} parses one Vortex .gen and returns the
#' data within as one data.frame.
#'
...
#' Collate Vortex .gen output files into one data.frame
#'
#' \code{collate_gen} collates all Vortex output files matching a given project
#' name (and scenario name when relevant) in a given directory into one
#' data.frame using \code{collate_one_gen}.
#'
#' The number of Vortex simulation runs has to be specified, as it cannot be
#' inferred by vortexR.
#'
...
ollate_gen <- function(project,
  ...
  if (save2disk) df2disk(d, dir_out, fname, "_gendata")
  return(d)
}